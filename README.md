...
| **`unauthorized_client` / "Invalid client or Invalid client credentials"** after login | The app is sending a **wrong or empty `ClientSecret`** at the token exchange (Keycloak returns `unauthorized_client` for a bad/blank secret). Same cause as above: a stray/empty `OpenIdConnect__ClientSecret` env var or a blank `.env` line is overriding `local-dev-secret`. Check the app's startup log line **`OIDC effective config -> ...`** — if it shows `ClientSecret=(EMPTY!)` or the wrong value, clear the override (see the row above) so `appsettings.Development.json` supplies `local-dev-secret`. It must match the client secret in `keycloak/cave-realm.json` (admin console: **Clients → amcos-local → Credentials**). |
