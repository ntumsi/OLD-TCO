...
| `007_full_coverage.sql` | **Fills the remaining empty web-feature tables** so every page shows data: `lookup.payplantags` (Military/DMDC/WASS — the `data.inventory` view filters on these); civilian inventory (`crunch.inventoryprocessed` + `crunch.wass_processed` → `data.inventory`, the Admin ▸ Inventory *development* side) plus `load_inventory.inventory_production` (created here; the *production* side); **CCE** costs (`"BLS_OES".occupationalemploymentstatisticsmetro` + MSA `warehouse.location` → `data.costscce`); Civilian PCS per-diem (`web.civlocationperdiem`); sample Admin ▸ Log rows; and a richer `webuser.amcosuser` set (+ login history) with `PendingSponsor`/`PendingAdmin`/`Denied`/`Active` statuses so Admin ▸ Approvals / Sponsor Action / Users filters have content |
