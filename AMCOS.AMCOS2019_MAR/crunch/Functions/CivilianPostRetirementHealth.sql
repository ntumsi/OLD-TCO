...
-- Description:	
-- =============================================
CREATE FUNCTION [crunch].[CivilianPostRetirementHealth]
(
    @PayPlan NVARCHAR(3)
)
RETURNS @Costs TABLE
...
BEGIN
    ...
    SELECT PayPlan,
           ...
           Step,
           PersonnelNumber,
           dbo.GetCostElementId(@PayPlan, 'Federal OM', 'Post Retirement Health Insurance') AS CostElementId,
           (
               SELECT paramValue
               FROM dataload.SingleValues
               WHERE PayPlan = 'GP'
                     AND paramName = 'postRetirementHealth'
           ) AS Amount
