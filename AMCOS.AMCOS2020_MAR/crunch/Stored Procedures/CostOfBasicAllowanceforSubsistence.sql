...
-- Dependencies: crunch.PayProcessed 
...
BEGIN
    ...
    (
        ...
        AvgAnnualPay NUMERIC(18, 2) NULL,
        AvgAnnualPayments NUMERIC(18, 2) NULL,
    );
    ...
    DECLARE @BAS_AO_AWO_Amt AS NUMERIC(18, 2);
    DECLARE @BAS_AE_Amt AS NUMERIC(18, 2);
    DECLARE @AO_AWO_ratio AS NUMERIC(18, 2);
    DECLARE @AE_ratio AS NUMERIC(18, 2);
    SET @BAS_AO_AWO_Amt = crunch.GetArmyBudgetSingleValue('BAS_Domestic_AO_AWO', 'MPA', 'Avg', @AmcosVersionId);
    SET @BAS_AE_Amt = crunch.GetArmyBudgetSingleValue('BAS_Domestic_AE', 'MPA', 'Avg', @AmcosVersionId);


    --compute the ratio
    SET @AO_AWO_ratio = @BAS_AO_AWO_Amt /
                        (
                            SELECT SUM(AvgAnnualPay)FROM #DMDCBAS WHERE PayPlan IN ( 'AO', 'AWO' )
                        );
    SET @AE_ratio = @BAS_AE_Amt /
                    (
                        SELECT SUM(AvgAnnualPay)FROM #DMDCBAS WHERE PayPlan IN ( 'AE' )
                    );
    ...
    UPDATE #DMDCBASGradeLevel
    SET Cost = crunch.GetSingleValue('AA', 'BAS_O_WO', @AmcosVersionId)
    WHERE PayPlan IN ( 'AO', 'AWO' );
    ...
    --the higher grade officers (generals) sometimes don't have any costs from DMDC
    --in those cases we just fill in with data from other officers in the same pay plan
    --since officer BAS is the same for all grade levels
    UPDATE #BASFinal
    SET Cost = b.Cost,
        CostElementId = b.CostElementId
    FROM #BASFinal AS a
        INNER JOIN
        (SELECT * FROM #BASFinal) AS b
            ON a.PayPlan = b.PayPlan
    WHERE a.CostElementId IS NULL;
