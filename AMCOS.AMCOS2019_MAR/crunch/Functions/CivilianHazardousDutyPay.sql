...
-- Description:	
-- =============================================
CREATE FUNCTION [crunch].[CivilianHazardousDutyPay]
(
    @PayPlan NVARCHAR(3)
)
RETURNS @Costs TABLE
...
BEGIN
    ...
    SELECT PayPlan,
           ...
           Step,
           PersonnelNumber,
           dbo.GetCostElementId(@PayPlan, 'Army CivPay', 'Civ Hazardous Duty Pay (6100.11H0)') AS CostElementId,
           SUM(AmountPaid) * 26 AS Amount
    FROM load_GFEBS.Processed
    WHERE PayPlan = @PayPlan
          AND CostElementCode IN ( '6100.11H0' )
    GROUP BY PayPlan,
             OccupationalGroupNumber,
             OccupationalSeriesNumber,
             StateCountry,
