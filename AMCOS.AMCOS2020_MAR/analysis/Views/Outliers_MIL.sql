CREATE VIEW [analysis].[Outliers_MIL]
AS
...
FROM
(
    SELECT PERCENTILE_DISC(.99)WITHIN GROUP(ORDER BY Amount) OVER (PARTITION BY CostElementName, a.GradeLevel, a.PayPlan) AS myperc,
           a.Amount,
           b.Inventory,
           e.Avg_StepYOS,
           e.agg_stepyos,
    ...
    WHERE a.PayPlan IN
          ...
          AND a.LocationId = -1 --can't compare non location to locaiton specific costs as locaiton specific are much higher (BAH, COLA, Overseas)
) AS a
