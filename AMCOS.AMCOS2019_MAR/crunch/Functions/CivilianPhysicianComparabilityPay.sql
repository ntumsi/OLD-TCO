...
-- Description:	
-- =============================================
CREATE FUNCTION [crunch].[CivilianPhysicianComparabilityPay]
(
    @PayPlan NVARCHAR(3)
)
RETURNS @Costs TABLE
...
BEGIN
    ...
    SELECT PayPlan,
           ...
           Step,
           PersonnelNumber,
           dbo.GetCostElementId(@PayPlan, 'Army CivPay', 'Civ Physician Comparability Pay (Market Pay) (6100.11T0)') AS CostElementId,
           SUM(AmountPaid) * 26 AS Amount
    FROM load_GFEBS.Processed
    WHERE PayPlan = @PayPlan
          AND CostElementCode IN ( '6100.11T0' )
    GROUP BY PayPlan,
             OccupationalGroupNumber,
             OccupationalSeriesNumber,
             StateCountry,
