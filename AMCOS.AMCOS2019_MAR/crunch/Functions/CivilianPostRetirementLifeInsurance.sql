...
-- Description:	
-- =============================================
CREATE FUNCTION [crunch].[CivilianPostRetirementLifeInsurance]
(
    @PayPlan NVARCHAR(3)
)
RETURNS @Costs TABLE
...
BEGIN
    ...
    SELECT PayPlan,
           ...
           Step,
           PersonnelNumber,
           dbo.GetCostElementId(@PayPlan, 'Federal OM', 'Post Retirement Life Insurance') AS CostElementId,
           (
               SELECT paramValue
               FROM dataload.SingleValues
               WHERE PayPlan = 'GP'
                     AND paramName = 'postRetirementLifeInsurance'
           ) AS Amount
